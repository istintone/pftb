...
(async () => {
  ...
  // ---------- スキル(D39 → docs/03 §3.21) ----------
  {
    ...
    for (const [name, fx] of Object.entries(E.SKILL_FX)) {
      // **数の効果を持たない札**(→docs/10 §3.57)。連携の扱いそのものを変えるので
  ...
  // ---------- 起点はマッチアップで決まる(D27 → docs/07 §7.8) ----------
  {
    ...
    // **1クラブで測らない**(→docs/10 §3.63)。DF と MF の差は名簿によっては
  ...
  // ---------- シュートの枝分かれ(D29 → docs/07 §7.9) ----------
  {
    ...
    // --- 相互カバー(→docs/10 §3.63)。**厚く構えた形ほど消耗が緩い** ---
    {
    ...
    // --- GKの飛び出し(→docs/10 §3.62)。**GKの死に能力に仕事がある** ---
    {
  ...
  // ---------- スタミナ(D30 → docs/07 §7.10) ----------
  {
    ...
    // ①''' **受けに行く動き**(→docs/10 §3.66)。近くの味方が tec/spd で顔を出して
