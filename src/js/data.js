...
const HELP={
  ...
  // **名指しで買える唯一の場所**(→docs/03 §3.67)。値段の根拠まで書く。
  ...
  // **監督とクラブは別の入れ物**(→docs/06 §6.50)。説明もそこで割る
...
/**
 * 秘書の名前(→docs/03 §3.43a)。**クラブごとに決まる**ので、移れば別の人になる。
 *
 * 絵は既にクラブごとに替わっていた(→docs/06 §6.27)のに、呼び名が「秘書」のままで
...
const TUNING={
  ...
  // 見本市(→docs/06 §6.56)。**n 枚を並べ、ms ごとに1枚ずつ入れ替える**
  fair:{ n:4, ms:2600 },
  scout:[
  ...
  mail:{ keep:30 },
  // CLUB NEWS の流し表示(→docs/06 §6.58)。**1行を ms ごとに入れ替える**
  news:{ ms:4200 },
  ...
  buy:{ k:3.5 },
  // 移籍市場(→docs/03 §3.67)。**名指しで買える唯一の経路**。
