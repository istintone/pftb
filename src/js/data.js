...
// **エントリーも打ち手の一つ**(→docs/03 §3.23)。カップの開催節にだけ現れる。
// その節の管理行動を大会参加に充てる、という扱いにしてある。
/**
...
const CHAT={
  ...
  whoAsk:"誰を呼びますか。",
  restSec:"全員に休養を伝えました。コンディションが戻り、痛みも引きやすくなります。",
...
const HANDS=[
  { id:"entry", icon:"🏆", label:"エントリー", cup:true,
    desc:"開催中のカップ戦に参加する。勝ち続ける限りリーグ戦は進められない", done:"エントリー" },
...
const TUNING={
  ...
  // クラブチャット(→docs/03 §3.29)。訓練/交流の手応え(効果は後で決める)
