...
const SKILL_FX={
  ...
  "正確なクロス":   { at:"origin",  grp:"cross",    s:1.12 },  // クロス(W型)の対
  "厳しい寄せ":     { at:"counter", grp:"press",    w:1.55, s:1.05 },  // タックル(S型)の対
  "推進力":         { at:"origin",  grp:"carryOut", s:1.12 },  // オーバーラップ(W型)の対
  "正確なフィード": { at:"origin",  grp:"long",     s:1.12 },  // ロングパス(W型)の対
  // ---- MF ----
  ...
  "パスの精度":     { at:"origin",  grp:"passTec",  s:1.12 },  // スルーパス(W型)の対
  "ミドルの精度":   { at:"finish",  grp:"far",      s:1.12 },  // ロングシュート(W型)の対
  "サイドチェンジ": { at:"origin",  grp:"wide",     s:1.12 },  // 展開力(W型)の対
  "間合いの読み":   { at:"counter", grp:"tec",      s:1.12 },  // ボール奪取(W型)の対
  // ---- FW ----
  ...
  "冷静なフィニッシュ":{ at:"finish",grp:"close",    s:1.12 },  // ゴール前の嗅覚(W型)の対
  "切れ込みの鋭さ": { at:"origin",  grp:"cut",      s:1.10 },  // カットイン(W型)の対
  "初速":           { at:"origin",  grp:"spd",      s:1.10 },  // スピード(W型)の対
  // ---- 汎用(ポジションを問わない) ----
...
const HELP={
  ...
  cards:()=>"集めた選手カードの一覧です。"
    ...
    +"<br>名前の右の<b>★</b>は訓練で覚醒した回数です（任期のあいだだけ）。"
  ...
  deck:()=>{
    ...
    return "ピッチの11枠に選手を並べます。"
      ...
      +"<br>名前の下の<b>★</b>は訓練で覚醒した回数です。"
      +"<br><b>白い線</b>は選手同士の<b>連携</b>で、太いほど噛み合っています。"
      +"試合と交流で積み上がり、<b>編成から外しても消えません</b>（戻せば続きから）。"
      +"消えるのは任期が明けたときだけです。"
  ...
  chat:"クラブのグループチャットです。ここで1節の準備をすべて決めます。"
    +"<br>秘書がカップ戦のエントリー、<b>相手の見立て</b>、打ち手(訓練・交流・休息)、"
  ...
  season:()=>{
    ...
    return "任期"+TUNING.tenure.limit+"節のカレンダーです。上から下へ進みます。"
      ...
      +"<br><b>"+c.name+"</b>は<b>"+c.every+"の倍数の節</b>に開催され、"
      +"<b>チーム熟練度 "+fmtNum(c.needExp)+" 以上</b>で参加できます。"
      +c.rounds+"回戦を勝ち抜けば優勝で <b>"+fmtNum(c.prize[0])+" コイン</b>、"
      +"<b>初優勝はトロフィー</b>になります。"
  ...
  clubhouse:()=>"監督個人の資産を置く場所です。名声とトロフィーはクラブを移っても失われません。"
    ...
    +"格上に勝つ・昇格する・タイトルを獲ると増え、<b>減ることはありません</b>。"
