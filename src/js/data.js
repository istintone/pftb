...
const SKILLS={
  ...
  MF:["スルーパス","視野の広さ","キープ力","ボール奪取","ロングシュート","ドリブル","展開力","運動量",
      "PKの名手","セカンドボール","パスの精度","ミドルの精度","サイドチェンジ","間合いの読み","捌き"],
...
const SKILL_FX={
  ...
  // `pass` の W型(→docs/08 §8.6③)。**長らく空けていた枠**。
  // 「38%と広いので W を置くと強くなりすぎる」と見送っていたが、実測すると
  // +0.005 で、ドリブル(+0.011)やスルーパス(+0.034)より**弱い**。見送る理由が無かった
  "捌き":           { at:"origin",  grp:"pass",     w:1.57, s:1.05 },
  ...
  // **同じ人物でも、カードが違えば固有スキルも別**(→docs/10 §3.13d)。
  ...
  // **保持して崩す面々**。起点は passTec と carry に寄せ、守りは press と tec で刻む。
  // (層,グループ,w,s)は上に並んでいる組から採り、個性は2枚目の k で出す(→上の注意書き)
  "足元の門番":     { sig:"valdes",
    fx:[{ at:"gk", k:1.10 },{ at:"vision", k:1.18 }] },
  "最終ラインの設計": { sig:"pique", move:"縦に差し込む",
    fx:[{ at:"origin", grp:"passTec", w:1.85, s:1.08 },{ at:"aerial", k:1.18 }] },
  "不屈の主将":     { sig:"puyol", move:"身体を投げ出す",
    fx:[{ at:"counter", grp:"press", w:1.60, s:1.10 },{ at:"block", k:1.18 }] },
  "静かな盾":       { sig:"abidal",
    fx:[{ at:"counter", grp:"tec", s:1.14 },{ at:"cover", k:1.18 }] },
  "両翼の走者":     { sig:"adriano", move:"外を上がる",
    fx:[{ at:"origin", grp:"cross", s:1.13 },{ at:"start", k:1.15 }] },
  "止まらない右":   { sig:"danialves", move:"右を蹂躙する",
    fx:[{ at:"origin", grp:"cross", s:1.13 },{ at:"spDeliver", k:1.15 }] },
  "刈り取る":       { sig:"mascherano", move:"前で潰す",
    fx:[{ at:"counter", grp:"tec", s:1.14 },{ at:"tough", k:1.12 }] },
  "間で消える":     { sig:"busquets", move:"間で受ける",
    fx:[{ at:"counter", grp:"all", s:1.05 },{ at:"vision", k:1.20 }] },
  "回し続ける":     { sig:"xavi", move:"三角形を作る",
    fx:[{ at:"origin", grp:"passTec", w:1.85, s:1.08 },{ at:"mid", k:1.10 }] },
  "継ぐ足元":       { sig:"thiago", move:"半歩で外す",
    fx:[{ at:"origin", grp:"carry", w:1.55, s:1.17, when:"fresh" },{ at:"recv", k:1.25 }] },
  "割って入る":     { sig:"keita", move:"遅れて入る",
    fx:[{ at:"finish", grp:"close", w:1.70, s:1.13 },{ at:"tough", k:1.12 }] },
  "無二の左":       { sig:"messibar", move:"三人を置き去る",
    fx:[{ at:"origin", grp:"carry", w:1.55, s:1.17, when:"fresh" },{ at:"onTarget", k:1.14 }] },
  "裏を取り続ける": { sig:"pedro", move:"背後へ抜ける",
    fx:[{ at:"finish", grp:"close", w:1.70, s:1.13 },{ at:"start", k:1.20 }] },
  "早すぎた芽":     { sig:"bojan", move:"流れを変える",
    fx:[{ at:"finish", grp:"close", w:1.70, s:1.13 },{ at:"joker", k:1.90 }] },
  "外から中へ":     { sig:"villa", move:"左から巻く",
    fx:[{ at:"finish", grp:"far", w:2.00, s:1.07 },{ at:"recv", k:1.25 }] },
};
/**
...
const SK_WHEN={
  ...
  // 毎ティック refreshStamina が選手に降ろしている(→docs/10 §3.50)
  lead: (p)=>!!p&&p.lead>0,
};
...
const HELP={
  ...
  // **名指しで買える唯一の場所**(→docs/10 §3.67)。値段の根拠まで書く。
  ...
  deck:()=>{
    ...
    return "ピッチの11枠に選手を並べます。"
      ...
      // **軸の説明はここに置く**(→docs/11 §6.37)。指名は試合中に行うが、
  ...
  season:()=>{
    return "任期"+TUNING.tenure.limit+"節のカレンダーです。上から下へ進みます。"
      ...
      +"<br>クラブの現況とオーナーの評価は <b>CLUB</b> に、"
      +"右端の<b>日程</b>で<b>大会の一覧と参加条件</b>を開けます。"
  ...
  // **監督とクラブは別の入れ物**(→docs/11 §6.50)。説明もそこで割る
  manager:()=>"<b>監督そのもの</b>の画面です。クラブを移っても失われないものだけが並びます。"
    +"<br>どの画面からでも<b>右上の顔</b>を押すと開きます。"
    +"<br>顔と背景はここで替えられます（⚙）。"
    +"<br><br><b>名声</b>が次にどのクラブから声がかかるかを決めます。"
    ...
    +"覚醒の★と連携を持ったまま付いてきます（→<b>BRIEFING</b> の説明）。"
    +"<br><br>記録の一戦(<b>MEMORABILIA</b>)は HOME の"
    +"<b>エキシビジョン</b>へ移しました。開いた記録はクラブを移っても残ります。"
    +"<br><br><b>CAREER</b> は季ごとの着地、<b>TROPHIES</b> は実績の棚です。"
    +"<b>カップ"+CUPS.length+"種と、各リーグ各部の制覇</b>が並び、"
    +"まだ獲っていないものも鍵つきで置いてあります。"
    +"同じ実績を重ねて獲ると<b>回数</b>だけ増え、初めて獲った季は残ります。"
    +"<br>いずれも<b>通算</b>です。いま預かっているクラブぶんだけの戦績は CLUB にあります。",
  // **日程に依らない一戦**(→docs/11 §6.66)
  exhibition:()=>"<b>節も日程も使わない試合</b>を集めた場所です。"
    +"いつ挑んでも、任期は1節も進みません。"
    +"<br><br><b>MEMORABILIA</b> は合言葉で開く<b>記録の一戦</b>です。"
    +"実在した強豪の11人がそのまま相手になります。"
    +"合言葉は世界の頂点を獲ったときにも届きます。"
    +"<br>勝てば<b>その11人の誰か</b>か、<b>限定の采配</b>が手に入ることがあります。"
    +"限定の采配は<b>そのクラブの選手にしか効きません</b>。"
    +"<br><br>開いた記録は<b>クラブを移っても残ります</b>。",
  mission:"クラブやキャリアの目標を並べる場所です。<b>まだ準備中</b>で、中身はこれから入ります。",
  clubhouse:()=>"<b>いま預かっているクラブ</b>の画面です。退任すると置いていくものが並びます。"
    +"<br><b>CLUB</b> はオーナーの目標と評価、順位、熟練度、資金、スポンサーです。"
    ...
    +"<br><br><b>CLUB RECORD</b> は<b>このクラブに来てからの戦績</b>だけを抜いたものです。"
    +"移籍すると畳まれ、戻ってきても前回ぶんは繋がりません。"
    +"監督としての通算は MANAGER にあります。",
  gallery:"各レアリティの見本です。<b>所持カードではありません</b>。タップで詳細が開きます。"
    +"<br>WORLD CLASS はプロスカウトからまれに出ます。"
    +"LEGENDS は実在選手をモチーフにする段のためパックからは出ず、"
    +"トロフィーや実績など別の経路で配ります。",
  secretary:"秘書からの連絡（<b>ANNOUNCE</b>）です。**溜まっていく連絡**で、"
    +"節の打ち合わせ（<b>BRIEFING</b>）とは別物です。"
...
const ORIGINS={
  ...
  // **一発で深くへ届く札(`to`)を pow が独占しない**(→docs/10 §3.64)。
...
const FACILITIES=[
  ...
  // **1段でも建っていれば機能する**(→docs/10 §3.57)。段が上がるほど上がってくる段が良くなる
...
/**
 * 特別采配(→docs/10 §3.50)。**指示(上の5つ)の上に1つだけ重ねる**。
 *
...
const TACTICS=[
  {
    ...
    // **強さも持たせる**(→docs/10 §3.50)。w だけだと札の選ばれ方しか変わらず、
...
// **報酬は引換券で配る**(→docs/10 §3.40a)。kind は TICKETS の id と同じ。
...
/**
 * 秘書の名前(→docs/10 §3.43a)。**クラブごとに決まる**ので、移れば別の人になる。
 *
 * 絵は既にクラブごとに替わっていた(→docs/11 §6.27)のに、呼び名が「秘書」のままで
 * **役職名でしか呼べない相手**だった。毎節いちばん多く言葉を交わす相手なので、
 * ここに名前があるだけで任期の手触りが変わる。
 *
 * **リーグの国から採る**。現地で雇った人、という筋がいちばん自然で、
 * クラブを移ったときに「土地が変わった」ことも名前で分かる。
 */
const SEC_NAMES={
  eng:{ first:["エマ","ソフィ","グレース","ルース","アリス","クロエ","ニナ","アイリス"],
        last:["ホイットフィールド","ハーパー","アシュコム","エラリー",
              "マーズデン","ベラミー","ラドクリフ","ソーンリー"] },
  esp:{ first:["エレナ","マルタ","ルシア","カルメン","パウラ","ヌリア","イレネ","アルバ"],
        last:["ソラーノ","ビダル","エレーラ","ボネット",
              "アギーレ","オルテガ","モリーナ","レケーナ"] },
  ita:{ first:["ジュリア","キアラ","エリーザ","サラ","マルタ","アレッシア","ビアンカ","ナディア"],
        last:["ロヴェッリ","サルトーリ","ベッリーニ","フォンターナ",
              "カルボーネ","マンクーゾ","リーヴァ","トレヴィザン"] },
  ger:{ first:["レナ","アンヤ","ミア","ゾフィー","カトリン","ネーレ","フライア","ヤナ"],
        last:["ブラント","ケラー","ヴェーバー","ホフマン",
              "リントナー","リヒター","ロイター","ヴォルフ"] },
  fra:{ first:["カミーユ","クロエ","アメリ","レア","マノン","サビーヌ","ノエミ","エリーズ"],
        last:["デュヴァル","モロー","ルフェーヴル","ロシェ",
              "ヴァスール","オベール","マルシャル","コラン"] },
  sam:{ first:["アナ","ベアトリス","カルラ","ルアナ","マリーナ","ハファエラ","タイス","ヴィトーリア"],
        last:["フェハス","ノゲイラ","ヴァスケス","コヘイア",
              "アウメイダ","バストス","ヒベイロ","サンパイオ"] },
};
...
const MAILS=[
  ...
  // 受信箱が上限(30通)を超えて古い案内が押し出された瞬間に連鎖が切れる。
  {
    ...
    text:"監督、就任おめでとうございます。{secFull}と申します。秘書として、これからお手伝いさせていただきます。"
...
const CHAT={
  ...
  // 節の出来事(→docs/10 §3.48)。**選手が話しかけてくる**ので、秘書ではなく本人の声
  ...
  ready:["すべて整いました。試合へ向かいましょう。","準備完了です。ピッチでお待ちしていますね。",
         "以上です。あとは戦うだけですね。"],
  // --- 監督。**単語で返さない**。短くても文にする(→docs/11 §6.23) ---
...
const TUNING={
  ...
  // 消耗(→docs/07 §7.10)。**相互カバー**(→docs/10 §3.63)がここに乗る。
  //   sup*   … 周りに味方が居るほど消耗が緩い。厚く構えた形ほど終盤に強くなる
  //            supH/supX は「近い」とみなす広がり、supK は効き、supMax は上限
  // オフザボールの走行量(→docs/10 §3.65)。**立つ位置で走る量が違う**。
  //   offBase … 走らない位置(中央CB・中央FW)の係数
  //   offMid  … 中盤の帯にいるほど乗る(攻守で往復する)
  //   offWide … 外にいるほど乗る(上下動が長い)
  // 3つの平均が概ね 1.0 になるよう置き、チーム全体の消耗量は変えない
  // **疲れ方は能力で違う**(→docs/10 §3.65)。stam^wear で効かせる。
  // 一律に掛けると、比で決まる判定では丸ごと相殺されて sta が仕事を失う
  // (実測: staReduce を 0.95 にしても、消耗を3倍にしても sta+6 は +0.08 のまま)。
  // 足と技から落ちて力は残る、という落ち方なら疲れたチームは「弱くなる」のではなく
  // 「形が変わる」ので、精度の判定(枠に飛ぶ・パスが通る)に差が出て相殺されない。
  wear:{ spd:1.35, tec:1.25, atk:1.00, def:0.90, pow:0.70, sta:0 },
  // **受けに行く動き**(→docs/10 §3.66)。ボールの近くにいる味方が、
  // tec と spd で顔を出してボール保持者を助ける。**wear が pow を終盤に
  // 残すぶん、こちらは元気なうちにしか出ない**ので、序盤は技と速さが、
  // 終盤は力が効く、という時間軸の住み分けになる。
  //   h,x  … どこまでを「近く」と見るか(高さ / 左右)
  //   ref  … 能力の基準値。平均的な選手がちょうど 1.0 になる値(= 加点 0)
  //   wear … 助ける側のスタミナの乗り方(1 より大きいほど早く足が止まる)
  //   tecW … 技と速さの配分(0.75 = 技7.5 : 速さ2.5)
  //   k    … 加点の強さ ／ max … 上下の頭打ち
  // **得点の水準は締め直さない**。th.shot で戻そうとすると、加点を受け取れない
  // 弱小クラブだけが痩せた(DIV3 1.94 → 1.84)。加点は DIV3 を傷つけていないので、
  // 締めないほうが正しい(実測 DIV1 2.82→3.01 / DIV2 2.22→2.36 / DIV3 1.94→1.95)
  // **密集度は見ない**(加重平均にする)。数で決めると基準そのものが底上げされ、
  // 能力の差が薄まってしまう。ここで問うのは「近くの味方が顔を出せるか」だけ
  offb:{ h:0.20, x:0.26, ref:12.57, wear:1.60, tecW:0.75, k:0.85, max:0.60 },
  fatigue:{ perMin:0.0032, perAct:0.0290, offBase:0.60, offMid:0.50, offWide:0.35, staReduce:0.45, minStam:0.30,
  ...
  // ユース(→docs/10 §3.57)。**任期に1人、タダで、ただし置いていく**。
  ...
  // 見本市(→docs/11 §6.56)。**n 枚を並べ、ms ごとに1枚ずつ入れ替える**
  fair:{ n:4, ms:2600 },
  scout:[
  ...
  // 特別采配の効きの倍率(→docs/10 §3.50)。**s と k の「1からの隔たり」を伸ばす**。
  ...
  shot:{ gapPow:0.45, deadZone:0.25, minRange:0.04, rangePow:1.00,
         ...
         feed:0.55,    // ゴールキックが味方に繋がる率(→docs/10 §3.61)
  ...
  // 節の出来事(→docs/10 §3.48)。**10試合に1回くらい**、何かが起きる。
  //   rate  … 起きる確率
  //   bond  … 個人練習で積む連携(組の値)
  ...
  // 采配を盗む(→docs/10 §3.51)。**その采配で戦ってきた相手とやり合う**と覚える。
  ...
  mail:{ keep:30 },
  // CLUB NEWS の流し表示(→docs/11 §6.58)。**1行を ms ごとに入れ替える**
  news:{ ms:4200 },
  // 経歴(→docs/03 §3.2.3)。**直近だけ見せる**。長く続けるほど伸びるので、
  // 全部並べると CLUB 画面が経歴だけで埋まる
  career:{ show:20 },
  // エンブレム(→docs/10 §3.54)。**ワールドクラブチャンピオンカップの獲得数**が★になる
  emblem:{ maxStar:3 },
  // メモラビリア(→docs/10 §3.55)。**普段は戦えない編成**との一戦。
  ...
  // 相手監督(→docs/10 §3.56)。tired … これを下回った選手から替える
  coach:{ tired:0.72 },
  // 相手の育ち具合(→docs/10 §3.53)。**段の数値は域内に収め、強さの差は★で出す**。
  ...
  // 実績の報酬(→docs/10 §3.52)。**初優勝にだけシグネチャが付く**。
  ...
  // カードの売却(→docs/10 §3.46)。**スカウトの元は取れない値**に置く。
  //   base   … 段ごとの下値
  ...
  buy:{ k:3.5 },
  // 移籍市場(→docs/10 §3.67)。**名指しで買える唯一の経路**。
