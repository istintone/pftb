...
function show(id,opts){
  ...
  // **画面が変わるたびに配り直す**。「見た」が増えたときだけ配ると、
  // オーナーのあいさつのように**画面の外で立つきっかけ**を拾えない
  if(S&&S.club&&S.player){
    // **タブ以外も覚える**(→docs/03 §3.43)。以前はタブ画面だけを立てていたので、
    // 市場やスカウトのような「入っていく画面」をきっかけにできなかった
    seenAll()[id]=1;
    if(mailTick())save();
  }
}
