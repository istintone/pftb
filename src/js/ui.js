...
/** ゴール。帯ごと金に染めて画面を揺らす。 */
function cutGoal(e,sc,as){
  return cutShow('<div class="cut goal">'
    +'<div class="cut-hd">'+e.hg+' - '+e.ag+'</div>'
    +'<div class="cut-row">'
      +cutFig(sc,e.side,"L win","ATK "+sc.c.atk)
      +(as?'<div class="cut-arrow">◀</div>'+cutFig(as,e.side,"R","アシスト"):"")
    +'</div>'
    +'<div class="cut-word goal">GOAL!!</div>'
  +'</div>',TUNING.play.goalMs," shake");
}
/** キックオフ。両クラブを向かい合わせる。 */
function cutKick(){
...
function mCut(e){
  ...
  switch(e.type){
    ...
    case "goal":    return cutGoal(e,by,e.assist&&mPlayer(_M,e.side,e.assist));
    case "save":    return cutVs(e,by,gk,"SAVE!",false);
    case "block":   return cutVs(e,by,vs,"BLOCK!",false);
    case "miss":    return Math.random()<P.cutMiss
                      ? cutShow('<div class="cut"><div class="cut-hd">SHOT</div>'
                        +'<div class="cut-row">'+cutFig(by,e.side,"L")+'</div>'
                        +'<div class="cut-word stop">枠を外れた…</div></div>',P.cutMs) : 0;
    case "origin":
    case "link":
      if(!vs)return 0;
